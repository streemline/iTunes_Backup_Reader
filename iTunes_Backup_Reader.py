...
def parseArgs():
        ...
        if not os.path.exists(output_dir):
                try:
                        ...
                        print(
                            f"Could not create output directory: {output_dir} Exception was: {str(ex)}"
                        )
                        sys.exit()
        ...
        '''Check outtype'''
        if out_type.lower() not in ["db", "csv", "txt"]:
                logger.error(f"Out type of {out_type} is not valid. Choose csv, db, or txt")
                sys.exit()
...
def main():
        ...
        if not bulk and not ir_mode:
                logger.info(f"Starting to read backup at: {input_dir}")
        ...
        if bulk:
                ...
                for folders in subfolders:
                        ...
                        logger.info(f"Starting to read backup at: {current_folder}")
        ...
        if ir_mode:
                ...
                for folders in all_paths:
                        ...
                        logger.info(f"Starting to read backup at: {folders}")
        ...
        end_time = time.time()
        logger.info(f"Program ended in: {str(end_time - start_time)} seconds")
